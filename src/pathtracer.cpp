...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  for (int i = 0; i < num_samples; i++)
  {
    ...
    // offset the origin of the ray from the hit point
    Ray sampled_ray = Ray(hit_p + EPS_D * wi_world, wi_world);
    Intersection intersect;
    if(bvh->intersect(sampled_ray, &intersect)) {
      L_out += intersect.bsdf->get_emission() * isect.bsdf->f(w_out, wi) * abs_cos_theta(wi);
    }
  }
  L_out = L_out / num_samples / pdf;
  return L_out;
...
Spectrum PathTracer::estimate_direct_lighting_importance(const Ray& r, const Intersection& isect) {
  ...
  for (SceneLight *light : scene->lights) {
    // calculate the number of samples to sample
    int num_samples;
    Spectrum L_tmp;
    ...
    for (int i = 0; i < num_samples; i++) {
      ...
      Spectrum l = light->sample_L(hit_p, &wi, &distToLight, &pdf);
      ...
      if (!bvh->intersect(shadow, &intersection)) {
        L_tmp += l * cos_theta(w_in) * isect.bsdf->f(w_out, w_in) / pdf;
      }
    }
    L_out += L_tmp / num_samples;
  }
...
Spectrum PathTracer::at_least_one_bounce_radiance(const Ray&r, const Intersection& isect) {
  ...
  if (r.depth > 1) {
    ...
    // initialize depth to max_ray_depth for before first recursion
    int depth = r.depth - 1;
    Ray ray = Ray(hit_p + EPS_D * w_in_world, w_in_world, depth);

    double cpdf = 0.6; 
    if ((depth == max_ray_depth && max_ray_depth > 1)|| coin_flip(cpdf)) {
      Intersection isect2;
      Spectrum L_later = Spectrum();
      if (bvh->intersect(ray, &isect2)) {
        if (isect.bsdf->is_delta()) {
          L_later = at_least_one_bounce_radiance(ray, isect2) + zero_bounce_radiance(ray, isect2);
        } else {
          L_later = at_least_one_bounce_radiance(ray, isect2);
        }
        L_out += L_later * sample * abs_cos_theta(w_in) / pdf / cpdf;
      } else {
        if (this->envLight) {
          L_later = this->envLight->sample_dir(ray);
          L_out += L_later * sample * abs_cos_theta(w_in) / pdf / cpdf;
        }
      }
    }
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  Vector2D pt = Vector2D();           // the randomly sampled point to evaluate
  double x_normalized, y_normalized;  // normalized pixel coordinates

  Spectrum spec = Spectrum();

  if (num_samples == 1) {
    x_normalized = (x + 0.5) / (sampleBuffer.w + 0.0);
    y_normalized = (y + 0.5) / (sampleBuffer.h + 0.0);
    
    Ray r = camera->generate_ray(x_normalized, y_normalized);

    // Vector2D rand = gridSampler->get_sample();
    // Ray r = camera->generate_ray_for_thin_lens(x_normalized, y_normalized, rand.x, rand.y);
    
    r.depth = max_ray_depth;
    spec += est_radiance_global_illumination(r);
  } else {
    float interval = 0.0, mu = 0.0, sigma = 0.0; // illum
    float s1 = 0.0; // sum_{k=1}^n x_k
    float s2 = 0.0; // sum_{k=1}^n x_k^2
    float illum = 0.0;
    Spectrum tmp;
    int n = 0;
    ...
    for (int i = 0; i < num_samples; i++)
    {
      pt = origin + gridSampler->get_sample();
      x_normalized = pt.x / (sampleBuffer.w + 0.0);
      y_normalized = pt.y / (sampleBuffer.h + 0.0);
      ...
      if (i % samplesPerBatch == 0) {
        mu = s1 / n;
        sigma = sqrt((s2 - s1 * s1 / n)/(n-1.0));
        interval = 1.96 * sigma / sqrt(n);
      }

      if (interval <= maxTolerance * mu) {
        break;
      }
    }
    spec = spec / n;
    sampleCountBuffer[x + frameBuffer.w * y] = n;
  }
